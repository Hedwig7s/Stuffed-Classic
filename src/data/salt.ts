...
export async function writeSalt(salt: string, path = `${CONFIG_PATH}/cachedsalt.txt`) {
    const file = Bun.file(path);
    await file.write(`${salt}\n${Date.now()}`);
}

export async function getSalt(length = 32, path = `${CONFIG_PATH}/cachedsalt.txt`): Promise<string> {
    const file = Bun.file(path);
